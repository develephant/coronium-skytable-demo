...
local b64_enc = require('skytable.utils').b64_enc
...
local function _generate_auth(key, action, utime)
  assert(key, "Key property is invalid.")
  assert(action, "Action property is invalid.")
  local auth_key = "$5$a15OvpxbCYXwckMF$tCAQzXa5d9KtVV9OIe17W0.4ZodDbitllW3Q0oD9BmA"
  local data = lower(key)..lower(action)..tostring(utime)
  local hash = b64_enc(hmac(crypto.sha1, data, auth_key, true))
  return hash
end
...
local function _create_request(key_path, action, data_path, payload, server_key, user_key, options)
  ...
  local headers = 
  {
    ["X-Coronium-Key"] = server_key,
    ["X-Coronium-UserKey"] = user_key,
    ["X-Coronium-Action"] = action,
    ["X-Coronium-Timestamp"] = utime,
    ["X-Coronium-Auth"] = _generate_auth(server_key, action, utime),
